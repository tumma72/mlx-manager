...
must_haves:
  truths:
    - "API keys persist securely and never appear in plain text in responses"
    - "Encrypted keys can be retrieved for use by backend services"
    - "Settings API returns credentials without exposing actual key values"
    - "Connection test validates API key with provider"
    - "Configuration changes take effect on next request without server restart"
  artifacts:
...
Purpose: Enable secure storage and retrieval of API keys (encrypted at rest) and provide CRUD endpoints for all configuration data that the frontend settings UI will consume. Configuration changes apply immediately (read from DB on each request) without requiring server restart.
...
**Hot-reload design note:** All endpoints read from and write to the database. Inference services already read configuration from the database on each request, so configuration changes take effect immediately without requiring a server restart.
...
sleep 2
curl -X POST http://localhost:10242/api/settings/providers \
...
1. Start server, configure a provider API key
2. Without restarting, make an inference request that routes to that provider
3. Verify the new configuration is used immediately
</verification>
...
- [ ] API keys never returned in responses (user-observable security)
- [ ] Connection test validates with real provider APIs (mocked in tests)
- [ ] Rule matching supports exact, prefix, and regex patterns
- [ ] Configuration changes apply without server restart (read from DB on each request)
