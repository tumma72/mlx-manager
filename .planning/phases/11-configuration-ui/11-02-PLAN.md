...
1. Create `frontend/src/lib/components/settings/ProviderForm.svelte`:
   ...
   - On save:
     1. Call `settings.createProvider()` with api_key
     2. If success, auto-test connection
     3. Call `onSave()` to refresh parent
   - On test: Call `settings.testProvider()` and update testResult
...
```
  ...
  <done>
ProviderForm component renders API key input with masking, save/test buttons, inline errors, TypeScript compiles.
  </done>
</task>
...
- [ ] Save button stores encrypted key (via backend)
- [ ] Test connection button validates with provider API
