...
must_haves:
  ...
  key_links:
    - from: "frontend/AuditLogPanel.svelte"
      to: "backend/api/v1/admin.py"
      via: "REST API and WebSocket connection"
      pattern: "ws://.*audit-logs"
...
<task type="auto">
  <name>Task 1: Add audit log API endpoints and WebSocket</name>
  <files>
    backend/mlx_manager/mlx_server/api/v1/admin.py
  </files>
  <action>
...
<task type="auto">
  <name>Task 2: Add API types and client functions</name>
  <files>
    frontend/src/lib/api/types.ts
    frontend/src/lib/api/client.ts
  </files>
  <action>
1. Update `frontend/src/lib/api/types.ts` to add audit log types:
   ```typescript
...
2. Update `frontend/src/lib/api/client.ts` to add audit log API functions:
   ```typescript
   // Add to existing client file

   // MLX Server base URL (different from manager)
   const MLX_SERVER_URL = 'http://localhost:10242';
   ...
   export async function getAuditLogs(filter: AuditLogFilter = {}): Promise<AuditLog[]> {
     ...
     const response = await fetch(`${MLX_SERVER_URL}/admin/audit-logs?${params}`);
     if (!response.ok) throw new Error('Failed to fetch audit logs');
     return response.json();
   }

   export async function getAuditStats(): Promise<AuditStats> {
     const response = await fetch(`${MLX_SERVER_URL}/admin/audit-logs/stats`);
     if (!response.ok) throw new Error('Failed to fetch audit stats');
     return response.json();
   }

   export function exportAuditLogs(filter: AuditLogFilter = {}, format: 'jsonl' | 'csv' = 'jsonl'): string {
     const params = new URLSearchParams();
     if (filter.model) params.set('model', filter.model);
     if (filter.backend_type) params.set('backend_type', filter.backend_type);
     if (filter.status) params.set('status', filter.status);
     ...
     return `${MLX_SERVER_URL}/admin/audit-logs/export?${params}`;
   }

   export function createAuditLogWebSocket(): WebSocket {
     return new WebSocket(`ws://localhost:10242/admin/ws/audit-logs`);
   }
   ```
  </action>
  <verify>
  ...
  <done>API types and client functions for audit logs exist</done>
</task>

<task type="auto">
  <name>Task 3: Create AuditLogPanel component and integrate into settings</name>
  <files>
    frontend/src/lib/components/settings/AuditLogPanel.svelte
    frontend/src/lib/components/settings/index.ts
    frontend/src/routes/(protected)/settings/+page.svelte
...
1. Create `frontend/src/lib/components/settings/AuditLogPanel.svelte`:
   ...
   <script lang="ts">
     ...
     function handleExport(format: 'jsonl' | 'csv') {
       ...
       const url = exportAuditLogs(filter, format);
...
3. Update `frontend/src/routes/(protected)/settings/+page.svelte`:
   Add an Audit Logs section at the bottom:
   ```svelte
   <script lang="ts">
  ...
  <done>AuditLogPanel component integrated into settings page</done>
</task>
...
4. Verify Audit Logs section appears with stats, filters, and table
5. Verify WebSocket connection indicator
6. Make an inference request and verify it appears in real-time
7. Test filters (model, backend, status)
8. Test export (JSONL and CSV)
</verification>

<success_criteria>
- GET /admin/audit-logs returns paginated logs with filters
- GET /admin/audit-logs/stats returns aggregate statistics
- GET /admin/audit-logs/export returns JSONL or CSV
- WebSocket /admin/ws/audit-logs streams live updates
