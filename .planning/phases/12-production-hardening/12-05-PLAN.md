...
<task type="auto">
  ...
  <action>
The frontend connects to the manager (port 8080), not directly to the MLX Server (port 10242). Add a WebSocket proxy endpoint so the frontend can access audit log streaming through the manager.
...
// This allows frontend to connect through the manager on port 5173 (dev) or 8080 (prod)
...
2. Start manager: `cd backend && uvicorn mlx_manager.main:app --port 8080`
