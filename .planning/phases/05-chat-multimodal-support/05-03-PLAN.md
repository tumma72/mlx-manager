...
must_haves:
  truths:
    - "Errors display in chat with red border and text"
...
2. **Store error with details:**
Change error state to store both summary and details:
```typescript
...
Parse errors to extract summary and details:
```typescript
// In catch block:
...
5. **Display error in messages area:**
Remove the old error banner (the red div above the input).
Instead, add error display in the messages list, after the assistant response or loading indicator:
...
6. **Import AlertCircle:**
```typescript
import { Send, Loader2, Bot, User, AlertCircle } from 'lucide-svelte';
```

7. **Clear error on profile change:**
In handleProfileChange, also reset:
```typescript
chatError = null;
```
  </action>
...
- Verify: Click to expand details
