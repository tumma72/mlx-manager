...
must_haves:
  ...
  key_links:
    - from: "chat/+page.svelte"
      ...
      pattern: "ContentPart"
    - from: "chat/+page.svelte isMultimodal"
      to: "selectedProfile.model_type"
      via: "derived state checking model_type === 'multimodal'"
      pattern: "model_type.*multimodal"
---
...
<task type="auto">
  ...
  <action>
Modify the chat page to add media attachment functionality.

**EXISTING STATE (already in chat page - do NOT duplicate):**
- `selectedProfile` derived from `runningProfiles.find((p) => p.id === selectedProfileId)` - USE THIS
- `error` state as `let error = $state<string | null>(null);` - USE THIS for validation errors
- Profile has `model_type` field with values: "lm" | "multimodal" | "whisper" | etc.
...
4. **Utility functions (use existing selectedProfile and error):**

```typescript
// Determine if current profile supports multimodal
// Uses EXISTING selectedProfile derived state
...
// Uses EXISTING error state for validation messages
...
- Max 3 attachments enforced with error message (uses existing error state)
