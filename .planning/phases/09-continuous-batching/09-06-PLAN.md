...
3. In `create_chat_completion`, add routing:
```python
...
4. Implement `_stream_batched_response` and `_complete_batched_response`:
   - Similar to existing handlers but using scheduler.submit()
   - Format output to match OpenAI response structure
  </action>
  <verify>
`cd backend && python -c "
...
"`
  ...
  <done>Chat endpoint routes through scheduler when batching enabled</done>
</task>
