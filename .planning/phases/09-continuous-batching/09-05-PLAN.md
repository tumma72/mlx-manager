...
5. Thread-safe wrapper:
   - `async generate_batch_step(requests: list[BatchRequest], sampler) -> dict`:
     - Use Queue-based threading pattern from inference.py
     - Run generate_tokens_for_batch in dedicated thread
     - Return results via Queue
  </action>
  <verify>
`cd backend && python -c "
from mlx_manager.mlx_server.services.batching.batch_inference import BatchInferenceEngine
# Just verify import works - actual test needs model
print('BatchInferenceEngine importable')
print('OK')
"`
  </verify>
  <done>BatchInferenceEngine with thread-safe batch generation</done>
</task>
...
# set_model exists but doesn't error without model
print('Integration structure OK')
"`
  </verify>
  <done>Scheduler integrated with BatchInferenceEngine for actual generation</done>
</task>
...
- MLX Metal thread affinity respected via dedicated thread
