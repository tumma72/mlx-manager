...
- `cd backend && uvicorn mlx_manager.main:app --port 10242` starts without error
- `curl http://localhost:10242/v1/models` returns JSON (may be empty list if no models loaded)
  </verify>
...
2. Verify routes work:
   ```bash
   # Manager routes
   curl http://localhost:10242/health
   curl http://localhost:10242/api/profiles

   # MLX Server routes (mounted at /v1)
   curl http://localhost:10242/v1/models
   curl http://localhost:10242/v1/health  # if exists
   ```
