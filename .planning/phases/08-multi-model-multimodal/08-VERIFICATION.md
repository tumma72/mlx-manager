...
|---|-------|--------|----------|
| 1 | Model pool manager supports multiple hot models with configurable memory limit | VERIFIED | `pool.py` (384 lines): `max_models=4`, `max_memory_gb` + `memory_limit_pct` params, `_ensure_memory_for_load()` checks available capacity before loading. `psutil.virtual_memory()` used for percentage-based limits. |
| 2 | LRU eviction unloads least-recently-used models when memory pressure detected | VERIFIED | `pool.py`: `_evict_lru()` finds `min(evictable, key=lambda m: m.last_used)`, removes from `_models` dict, calls `clear_cache()`. `_ensure_memory_for_load()` loops eviction until enough memory is available, raises HTTP 503 if no evictable models remain. Preloaded models are exempt via `_evictable_models()` filter. 24 unit tests pass covering eviction logic. |
| 3 | Vision models load via mlx-vlm and generate responses using mlx_vlm.generate() | VERIFIED | `pool.py` line 261-266: detects VISION type, imports `mlx_vlm.load()`, stores (model, processor). `vision.py` (344 lines): imports `mlx_vlm.generate` in `run_generation()`, calls with model/processor/images/max_tokens/temp. Both streaming and non-streaming paths use `vlm_generate()`. Chat endpoint routes images to vision service. |
| 4 | Embedding models load via mlx-embeddings for /v1/embeddings endpoint | VERIFIED | `pool.py` line 267-274: detects EMBEDDINGS type, imports `mlx_embeddings.utils.load()`. `embeddings.py` service (142 lines): forward pass via `model(input_ids, attention_mask)`, extracts `text_embeds` (L2-normalized). `api/v1/embeddings.py` endpoint validates model type, calls service, returns OpenAI-compatible `EmbeddingResponse`. Router registered in `__init__.py`. |
| 5 | Qwen, Mistral, and Gemma adapters handle their respective model families | VERIFIED | `qwen.py`: ChatML format with `<\|im_end\|>` stop token. `mistral.py`: system message prepending for v1/v2 compat, `</s>` stop. `gemma.py`: `<end_of_turn>` stop token. All registered in `registry.py` with auto-detection from model ID. 17 adapter unit tests pass. |
| 6 | Admin endpoints allow explicit model preload/unload | VERIFIED | `admin.py` (160 lines): `GET /admin/models/status` returns pool state with all model metadata. `POST /admin/models/load/{model_id}` calls `pool.preload_model()` (marks as eviction-protected). `POST /admin/models/unload/{model_id}` calls `pool.unload_model()` with 404 on not-found. Router registered in `__init__.py`. 7 admin unit tests pass. |
...
| `backend/mlx_manager/mlx_server/models/adapters/qwen.py` | Qwen adapter with ChatML stop tokens | VERIFIED | 53 lines, `<\|im_end\|>` stop token, graceful fallback |
| `backend/mlx_manager/mlx_server/models/adapters/mistral.py` | Mistral adapter with system message handling | VERIFIED | 64 lines, system message prepending for v1/v2 compatibility |
| `backend/mlx_manager/mlx_server/models/adapters/gemma.py` | Gemma adapter with end_of_turn stop | VERIFIED | 51 lines, `<end_of_turn>` stop token, graceful fallback |
...
| `vision.py` | `mlx_vlm.generate` | Direct call in thread | WIRED | Line 141 imports, line 154 calls `vlm_generate()` with all params |
...
| `pool.py` | `mlx_embeddings.utils.load` | Conditional import + call | WIRED | Line 269 imports, line 273 calls for EMBEDDINGS type |
...
|------|------|---------|----------|--------|
| `services/vision.py` | 131 | `TODO: Investigate mlx-vlm internals for true token-by-token streaming.` | INFO | Not a blocker — simulated streaming is a deliberate design choice documented in decisions. The current implementation yields the full response as a single content chunk, maintaining API compatibility. |
...
| Gate | Result |
|------|--------|
| ruff lint (all 15 files) | PASS — zero violations |
| mypy (with --ignore-missing-imports) | PASS — 30 source files, no issues |
| Unit tests (64 tests) | PASS — 64 passed, 3 warnings (deprecation + logfire config) |
...
- Pool manager (pool.py, 384 lines) implements multi-model support with LRU eviction, preload protection, and configurable memory limits
...
- Three new adapters (qwen.py, mistral.py, gemma.py) implement the ModelAdapter protocol with family-specific chat templates and stop tokens, all registered in registry.py
...
All 64 unit tests pass. Ruff and mypy quality gates pass. The single TODO in vision.py is an informational note about a future enhancement, not a missing implementation.
