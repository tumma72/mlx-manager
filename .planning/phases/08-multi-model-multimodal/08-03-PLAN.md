...
# Vision from config
assert detect_model_type('test', config={'vision_config': {}}) == ModelType.VISION
assert detect_model_type('test', config={'image_token_id': 123}) == ModelType.VISION
assert detect_model_type('test', config={'model_type': 'qwen2_vl'}) == ModelType.VISION

# Embeddings from config
assert detect_model_type('test', config={'architectures': ['BertForSentenceEmbedding']}) == ModelType.EMBEDDINGS
...
```
  ...
  <done>detect_model_type() correctly identifies vision, embeddings, and text-gen models from config and name patterns</done>
</task>
...
assert 'ModelType.EMBEDDINGS' in source, 'EMBEDDINGS handling not in _load_model'
...
def create_base64_uri(width, height):
    img_bytes = create_test_image(width, height)
    b64 = base64.b64encode(img_bytes).decode()
    return f'data:image/png;base64,{b64}'

# Test small image (no resize)
small_uri = create_base64_uri(100, 100)
img = asyncio.run(preprocess_image(small_uri))
assert img.size == (100, 100), f'Expected (100, 100), got {img.size}'

# Test large image (should resize)
large_uri = create_base64_uri(4000, 3000)
img = asyncio.run(preprocess_image(large_uri, max_dimension=2048))
assert max(img.size) <= 2048, f'Expected max dim <= 2048, got {max(img.size)}'

# Test batch
uris = [create_base64_uri(100, 100), create_base64_uri(200, 200)]
images = asyncio.run(preprocess_images(uris))
assert len(images) == 2
...
```
  ...
  <done>Image processor service created with base64/URL/file support and auto-resize, tested inline</done>
</task>
...
# Run inline verification from task verify sections
python -c "
from mlx_manager.mlx_server.models.detection import detect_model_type
from mlx_manager.mlx_server.models.types import ModelType
from mlx_manager.mlx_server.services.image_processor import preprocess_image, preprocess_images
from mlx_manager.mlx_server.schemas.openai import extract_content_parts, ChatMessage
print('All imports successful - infrastructure ready')
"
```
