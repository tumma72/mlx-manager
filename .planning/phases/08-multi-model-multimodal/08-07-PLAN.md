...
<task type="auto">
  ...
  <verify>
    Run adapter tests: `cd backend && pytest tests/mlx_server/test_adapters.py -v`
    Run type checking: `cd backend && mypy mlx_manager/mlx_server/models/adapters/`
    Smoke test: Manually verify with a vision model (e.g., Qwen-VL) that it no longer raises AttributeError
  </verify>
  <done>
...
<task type="auto">
  ...
  <verify>
    Smoke check: `cd backend && python -c "from mlx_manager.mlx_server.services.image_processor import DEFAULT_HEADERS; assert 'User-Agent' in DEFAULT_HEADERS; print('OK')"`
  ...
  <done>
    ...
    Smoke check passes and type checking clean.
  </done>
</task>
...
<verification>
1. All adapter tests pass: `cd backend && pytest tests/mlx_server/test_adapters.py -v`
2. User-Agent smoke check: `cd backend && python -c "from mlx_manager.mlx_server.services.image_processor import DEFAULT_HEADERS; assert 'User-Agent' in DEFAULT_HEADERS"`
