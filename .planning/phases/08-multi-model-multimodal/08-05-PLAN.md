...
must_haves:
  ...
  key_links:
    ...
    - from: "api/v1/__init__.py"
      ...
      via: "include_router in v1_router"
      pattern: "v1_router.include_router.*embeddings"
---
...
<task type="auto">
  <name>Task 3: Create /v1/embeddings endpoint and register in v1_router</name>
  <files>
backend/mlx_manager/mlx_server/api/v1/embeddings.py
backend/mlx_manager/mlx_server/api/v1/__init__.py
  </files>
  <action>
**Part A: Create embeddings.py API endpoint:**
...
**Part B: Register embeddings router in api/v1/__init__.py:**

Update `backend/mlx_manager/mlx_server/api/v1/__init__.py` to include the embeddings router:
...
IMPORTANT: The embeddings router does NOT have a prefix (no `prefix="/v1"`) because it will be included in v1_router which is already mounted at the appropriate level. The route will be `/v1/embeddings` through the v1_router. This follows the established pattern used by chat, completions, and models routers.
  </action>
  <verify>
```bash
cd /Users/atomasini/Development/mlx-manager/backend
...
print('Embeddings router tags:', router.tags)
print('Routes:', [r.path for r in router.routes])

# Verify v1_router includes embeddings
from mlx_manager.mlx_server.api.v1 import v1_router
routes = [r.path for r in v1_router.routes if hasattr(r, 'path')]
assert '/embeddings' in routes, f'Embeddings route not in v1_router. Routes: {routes}'
print('Embeddings route registered in v1_router:', '/embeddings' in routes)
"
```
Should show embeddings route in v1_router
  </verify>
  <done>
- /v1/embeddings endpoint created with model type validation
- Embeddings router registered through v1_router (consistent with chat, completions, models)
  </done>
</task>

<task type="auto">
  <name>Task 4: Add unit tests for embeddings</name>
...
- Embeddings router registered through v1_router (consistent pattern)
- Unit tests pass
</success_criteria>
