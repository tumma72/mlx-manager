...
must_haves:
  truths:
    - "/v1/messages endpoint accepts Anthropic-format requests"
    - "Streaming uses Anthropic SSE format with named event types"
    - "Non-streaming returns Anthropic-format response"
    - "Protocol translator converts request to internal format"
    - "Response includes proper stop_reason and usage"
  artifacts:
...
async def _handle_streaming(
    ...
    async def generate_events() -> Any:
        ...
        # 5. message_delta event (final usage)
        yield {
            "event": "message_delta",
            "data": json.dumps({
                "type": "message_delta",
