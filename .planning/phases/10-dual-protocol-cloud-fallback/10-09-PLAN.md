...
**Current state of _handle_text_request (lines 156-183):**
```python
async def _handle_text_request(
    ...
    if settings.enable_batching:  # <-- LINE 167
        try:
...
1. Add import at top of file (around line 23-31):
```python
from mlx_manager.mlx_server.services.cloud import get_router
```

2. Modify `_handle_text_request` function - insert routing check BEFORE the batching check at line 167.

**Replace lines 156-183 with:**
```python
async def _handle_text_request(
    request: ChatCompletionRequest,
) -> EventSourceResponse | ChatCompletionResponse:
    ...
    # INSERT THIS BLOCK BEFORE the existing batching check
...
3. Add new `_handle_routed_request` function (insert after `_handle_text_request`, around line 184):
```python
...
Note: `time`, `Any`, and `cast` are already imported in chat.py.
  </action>
  <verify>
cd /Users/atomasini/Development/mlx-manager/backend && python -c "
