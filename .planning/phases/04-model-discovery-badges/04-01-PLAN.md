...
<task type="auto">
  ...
  <verify>
    ...
    cd backend && source .venv/bin/activate && uvicorn mlx_manager.main:app --port 10242 &
    sleep 3
    ...
    curl -s http://localhost:10242/api/models/config/mlx-community/Qwen2.5-0.5B-Instruct-4bit \
    ...
    curl -s http://localhost:10242/api/models/local \
      -H "Authorization: Bearer $(cat /tmp/test_token 2>/dev/null || echo 'test')" | head -c 500
